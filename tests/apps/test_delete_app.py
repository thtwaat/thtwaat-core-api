...
def test_delete_app(client):
    ...
    delete_resp = client.delete(f"/api/v1/apps/{app_id}", headers=headers)
    assert delete_resp.status_code == 200
    
    get_resp = client.get(f"/api/v1/apps/{app_id}", headers=headers)
    if get_resp.status_code == 200:
        assert get_resp.json()["status"].lower() == "inactive"
    else:
        assert get_resp.status_code in [403, 404]

def test_delete_missing_app(client):
    headers, _ = get_auth(client)
    fake_id = str(uuid.uuid4())
    delete_resp = client.delete(f"/api/v1/apps/{fake_id}", headers=headers)

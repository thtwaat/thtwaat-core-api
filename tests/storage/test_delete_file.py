...
def test_delete_file(client):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as tmp:
        tmp.write(b"To be deleted")
        tmp_path = tmp.name
        
    try:
        with open(tmp_path, "rb") as f:
            upload_resp = client.post("/api/v1/storage/upload", files={"file": ("del.txt", f, "text/plain")}, headers=headers)
...
def test_delete_missing_file(client):
    ...
    del_resp = client.delete(f"/api/v1/storage/{fake_id}", headers=headers)

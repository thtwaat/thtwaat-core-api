...
def test_download_file(client):
    ...
    try:
        with open(tmp_path, "rb") as f:
            upload_resp = client.post("/api/v1/storage/upload", files={"file": ("test.txt", f, "text/plain")}, headers=headers)
        assert upload_resp.status_code == 201
        file_id = upload_resp.json()["id"]
        
        # FastAPI testclient follow_redirects
        download_resp = client.get(f"/api/v1/storage/{file_id}/download", headers=headers, follow_redirects=False)
        assert download_resp.status_code in [200, 307, 302, 301, 303, 308]
    finally:
        os.remove(tmp_path)

...
actor AgentsRepository {
    ...
    func widget(id: String) async -> APIResult<WidgetConfigResponse> {
        await client.get("/api/v1/agents/\(id)/widget")
    }

    func embed(id: String) async -> APIResult<EmbedSnippetResponse> {
        await client.get("/api/v1/agents/\(id)/embed")
    }

    func updateWidget(id: String, welcome: String, primaryColor: String, agentName: String?) async -> APIResult<WidgetConfigResponse> {
        await client.patch(
            "/api/v1/agents/\(id)/widget",

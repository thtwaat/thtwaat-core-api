...
def _pg_enum(enum_cls: type[enum.Enum], name: str):
    """Persist/compare Python Enum *values* (active), not names (ACTIVE)."""
    return SAEnum(
        enum_cls,
        name=name,
...
class Subscription(Base, TimestampMixin):
    ...
    provider                 = Column(_pg_enum(SubscriptionProvider, "subscription_provider_enum"), nullable=False)
    ...
    # State
    status               = Column(
        _pg_enum(SubscriptionStatus, "subscription_status_enum"),
        nullable=False,
